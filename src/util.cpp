...
const char * const BITCOIN_PID_FILENAME = "mocacoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "mocacoin" is a composite category enabling all Mocacoin-related debug output
            if(ptrCategory->count(string("mocacoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MocacoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MocacoinCore
    // Mac: ~/Library/Application Support/MocacoinCore
    // Unix: ~/.mocacoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MocacoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MocacoinCore";
#else
    // Unix
    return pathRet / ".mocacoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty mocacoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override mocacoin.conf
