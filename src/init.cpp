...
#include "config/mocacoin-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, mempoolrej, net, proxy, prune, http, libevent, tor, zmq, "
                             "mocacoin (or specifically: privatesend, instantsend, masternode, spork, keepass, mnpayments, gobject)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Mocacoin specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N MOCACOIN anonymized (default: %u)"), DEFAULT_PRIVATESEND_AMOUNT));
    ...
    strUsage += HelpMessageOpt("-instantsenddepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-1983, default: %u)"), DEFAULT_INSTANTSEND_DEPTH));
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The Mocacoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Mocacoin Core is running in a usable environment with all
...
void InitParameterInteraction()
{
    ...
    if (nLiqProvTmp > 0) {
        ...
        mapArgs["-privatesendrounds"] = "19839";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -privatesendrounds=19839\n", __func__, nLiqProvTmp);
        mapArgs["-privatesendamount"] = "198399";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -privatesendamount=198399\n", __func__, nLiqProvTmp);
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Mocacoin Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize Mocacoin Core.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Mocacoin Core is shutting down."));
...
#endif
    // Make sure only a single Mocacoin Core process is using the data directory.
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Mocacoin Core is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Mocacoin Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Mocacoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Mocacoin Core to complete") << "\n";
    ...
    nPrivateSendRounds = std::min(std::max(nPrivateSendRounds, 2), nLiquidityProvider ? 19839 : 16);
    nPrivateSendAmount = GetArg("-privatesendamount", DEFAULT_PRIVATESEND_AMOUNT);
    nPrivateSendAmount = std::min(std::max(nPrivateSendAmount, 2), 198399);
    ...
    // ********************************************************* Step 11c: update block tip in Mocacoin modules
    ...
    // ********************************************************* Step 11d: start mocacoin-privatesend thread
